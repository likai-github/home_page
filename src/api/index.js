...
class ApiClient {
  ...
  async request(endpoint, options = {}) {
    ...
    // 401：token 失效，清除状态并跳登录（仅管理类操作才会触发）
  ...
  async getPost(id) { return this.request(`/posts/${id}`); }
  ...
  // ── 数据库管理 ────────────────────────────────────────────────────────────
  async getDatabaseStatus() { return this.request('/database/status'); }
  async getDatabaseTables() { return this.request('/database/tables'); }
  async getTableColumns(tableName) { return this.request(`/database/tables/${tableName}/columns`); }
  async getTableSampleData(tableName) { return this.request(`/database/tables/${tableName}/data`); }
  async getMigrationHistory() { return this.request('/database/migrations'); }
  async runMigration() { return this.request('/database/migrate', { method: 'POST' }); }
  async resetDatabase() { return this.request('/database/reset', { method: 'POST' }); }
